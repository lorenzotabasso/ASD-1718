...
class EditDistance {
    ...
    /**
     * Recursive method that computes the edit distance between the 
     * first string (str1) and the second string (str2).
     * More specifically, it finds the minimum number of 
     * deletion and insertion to transform the second string 
     * in the first string.
     *
     * @param s1 first string.
     * @param s2 second string.
     *
     * @return the edit distance between the two given strings 
     *         (based only on insertion and deletion).
     */
    ...
    /**
     * Iterative Bottom-up version of a Dynamic Programming method that 
     * computes the edit distance between the first string (str1) and 
     * the second string (str2). More specifically, it finds the 
     * minimum number of deletion and insertion to transform
    ...
    static int editDistanceDynBottomUp(String str1, String str2) {
        ...
        for (int i = 0; i <= m; i++) {
            for (int j = 0; j <= n; j++) {
                ...
                // (3) store the minimum in editDistanceMatrix[i][j].
                else {
                    int dCanc = 1 + editDistanceMatrix[i][j - 1];
                    int dIns = 1 + editDistanceMatrix[i - 1][j];
    ...
    /**
     * Recursive Top-down version of a Dynamic Programming method that 
     * computes the edit distance between the first string (str1) and 
     * the second string (str2). More specifically, it finds 
    ...
    static int editDistanceDynTopDown(String str1, String str2) {
        ...
        int[][] editDistanceMatrix = new int[str1.length() + 1][str2.length() + 1];

        // Filling the matrix cells with -1 
        ...
        return editDistDynRec(str1, str2, str1.length(), str2.length(), editDistanceMatrix);
    }

    /**
     * Recursive method called in editDistanceDynTopDown, 
     * computes the edit distance between the first string (str1) and 
     * the second string (str2). More specifically, it finds the 
     * minimum number of deletion and insertion to transform
     * the second string in the first string.
     ...
     * @param eDM a integer matrix in which store the result 
     *            of all the recursive calls
     *
     * @return the edit distance between the two given strings 
     *         (based only on insertion and deletion).
     * @see #editDistanceDynTopDown(String, String) for more information.
     */
    private static int editDistDynRec(String str1, String str2, int m, int n, int[][] eDM) {
        ...
        // Its value memorized in the matrix
        else if (eDM[m - 1][n - 1] != -1) {
            return eDM[m - 1][n - 1];
        }

        // Last characters are the same, ignore then and
        // compute for remaining strings.
        // Store the returned value at eDM[m-1][n-1]
        else if (str1.charAt(m - 1) == str2.charAt(n - 1)) {
            return eDM[m - 1][n - 1] = editDistDynRec(str1, str2, m - 1, n - 1, eDM);
        }

        // If last characters are not same,
        // (1) compute dCanc and dIns
        // (2) find the minimum between dCanc and dIns
        // (3) store the minimum in eDM[m-1][n-1].
        else {
            int dCanc = editDistDynRec(str1, str2, m - 1, n, eDM); // Remove
            int dIns = editDistDynRec(str1, str2, m, n - 1, eDM); // Insert
            return eDM[m - 1][n - 1] = 1 + Math.min(dIns, dCanc);
        }
    }
}
